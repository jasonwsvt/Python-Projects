...
# Python Ver:   3.5.1
#
# Author:       Daniel A. Christie (altered for student tracking by Jason Storer)
#
...
def onSelect(self,event):
    ...
    with conn:
        ...
        for data in varBody:
            self.txt_fname.delete(0,END)      #delete clears the current contents
...
def onUpdate(self):
    ...
    if (len(var_phone) > 0) and (len(var_email) > 0): # ensure that there is data present
        ...
        with conn:
            ...
            count = cur.fetchone()[0]
            print(count)
            cur.execute("""SELECT COUNT(col_email) FROM tbl_student_tracking WHERE col_email = '{}'""".format(var_email))
            ...
            if count == 0 or count2 == 0: # if proposed changes are not already in the database, then proceed
                ...
                if response:
                    ...
                    with conn:
                        cursor = conn.cursor()
                        cursor.execute("""UPDATE tbl_student_tracking SET col_phone = '{0}',col_email = '{1}' WHERE col_fullname = '{2}'""".format(var_phone,var_email,var_value))
                        onClear(self)
                        conn.commit()
                else:
