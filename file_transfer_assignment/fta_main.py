...
class ParentWindow(Frame): #Frame is the primary tkinter object
    def __init__(self, master, *args, **kwargs):
        ...
        width = 456
        height = 247
        self.master.minsize(width,height) #(Height, Width)
        self.master.maxsize(width,height)
        # This CenterWindow method will center our app on the user's screen
        fta_func.center_window(self,width,height)
        self.master.title("File Transfer")
        self.master.configure(bg="#F0F0F0")
        # This protocol method is a tkinter built-in method to catch if 
        # the user clicks the upper corner, "X" on Windows OS.
        self.master.protocol("WM_DELETE_WINDOW", lambda: fta_func.ask_quit(self))
        arg = self.master

        # load in the GUI widgets from a separate module, 
        # keeping your code comparmentalized and clutter free
        fta_gui.load_gui(self)
